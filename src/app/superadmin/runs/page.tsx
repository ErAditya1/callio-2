...
export default function RunsPage() {
    ...
    useEffect(() => {
        // Fetch runs when auth and superuser status are verified and when page/sort changes
        if (auth.isAuthenticated && isSuperuser) {
            fetchRuns(currentPage, appliedFilters, false, sortBy, sortOrder);
        }
    }, [currentPage, auth.isAuthenticated, isSuperuser, appliedFilters, fetchRuns, sortBy, sortOrder]);

    // Auto-refresh every 5 seconds when enabled and filters are active
    useEffect(() => {
        // Only set up interval if superuser, auto-refresh is enabled, and there are applied filters
        if (!isSuperuser || !autoRefresh || appliedFilters.length === 0) {
            return;
        }
